...
echo "Memastikan Apache dan alat pendukung terinstal..."
apt update && apt install -y apache2 apt-mirror dpkg-dev

REPO_DIR="/var/www/html/repo"
CACHE_DIR="/repo-cache"

echo "Menghapus paket lama di repository..."
rm -rf "$REPO_DIR"
mkdir -p "$REPO_DIR"
chmod -R 755 "$REPO_DIR"

echo "Menghapus cache lama dan menyiapkan direktori baru..."
rm -rf "$CACHE_DIR"
mkdir -p "$CACHE_DIR"

echo "Mengunduh paket-paket LAMP dan dependensinya..."
cd "$CACHE_DIR"
...
echo "Memindahkan paket terbaru ke repository..."
mv *.deb "$REPO_DIR/"

echo "Membuat ulang indeks repository..."
cd "$REPO_DIR"
rm -f Packages.gz
dpkg-scanpackages . /dev/null | gzip -9c > Packages.gz

echo "Memberikan izin yang sesuai..."
